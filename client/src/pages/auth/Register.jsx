...
const Register = () => {
    ...
    const handleSendCode = async (e) => {
        e.preventDefault();
        if (!formData.phone) return setError('יש להזין מספר טלפון');
        if (!formData.region) return setError('יש לבחור אזור');
        if (formData.role === 'teacher' && teacherLicenses.length === 0)
            return setError('יש לבחור לפחות סוג רישיון אחד');
        ...
        try {
            const data = await authService.sendCode({ ...formData, license_type });
    ...
    const handleVerify = async (e) => {
        ...
        try {
            ...
            if (data.userId) {
                navigate('/login', { state: { message: 'נרשמת בהצלחה! התחבר עכשיו' } });
            } else {
                setError(data.message);
            }
        } catch {
        ...
        <div className="auth-container">
            <div className="auth-card">
                <div className="auth-logo">
                    <img src="/logo.png" alt="RoadMaster" style={{ width: '52px', height: '52px', borderRadius: '50%', objectFit: 'cover' }} />
                ...
                {step === 1 && (
                    <div className="step-container">
                        ...
                        <form onSubmit={handleSendCode} className="auth-form">
                            ...
                            <div className="form-group">
                                ...
                                <input type="password" name="password" value={formData.password}
                                    onChange={handleChange} placeholder="עד 8 אותיות אנגליות" required />
                            </div>

                            <div className="form-group">
                                <label>מספר טלפון</label>
                                <input type="tel" name="phone" value={formData.phone}
                                    onChange={handleChange} placeholder="050-0000000" required />
                            </div>
                        ...
                        <p className="auth-switch">יש לך חשבון? <Link to="/login">התחבר כאן</Link></p>
                    </div>
                )}
                ...
                {step === 2 && (
                    <div className="step-container">
                        ...
                        <p className="auth-subtitle">שלחנו קוד אימות ל־<strong>{formData.email}</strong></p>
                        ...
                        <form onSubmit={handleVerify} className="auth-form">
                            ...
                            <button type="button" className="auth-button resend-btn" disabled={loading}
                                onClick={async () => {
                                    setLoading(true);
                                    setError('');
                                    try {
